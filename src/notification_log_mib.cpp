	...
	if (((nlmConfigLogAdminStatus*)my_row->get_nth(2))->get_state() == 1) {
        ...
        snmpNotifyFilterEntry* snmpNotifyFilterEntry =
                ...
                if (snmpNotifyFilterEntry) {
                    List<MibTableRow>* r =
                      snmpNotifyFilterEntry->get_rows_cloned(&index, rowActive);
                    if (r->size() > 0) {
                            l = 2;
                    }
                    else {
				   ...
				   const OctetStr& engineID)
{
	...
	for (cur.init(logs); cur.get(); cur.next()) {
		...
		LOG_BEGIN(loggerModuleName, EVENT_LOG | 6);
		LOG("Notification logged (nlmLogIndex)");
		LOG(newIndex.get_printable());
		LOG_END;
		Counter32 ll = 0;
		if (s) {
		...
		for (cur.lookup(&ind);
             (((cur.get()) &&
		       (cur.get()->get_index().cut_right(1)==index)) ||
                     ((cur.next()) && (cur.get()->get_index().cut_right(1)==index)));) {
			MibTableRow* victim = cur.get();
			cur.next();
...
{
	Oidx newIndex(index);
	newIndex += (unsigned long)varIndex;
	MibTableRow* r = add_row(newIndex);
