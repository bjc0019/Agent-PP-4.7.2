 ...
 * @param engine_id
 *    the agent's own engine ID.
 */
void initCommunities(Mib* mib, const OctetStr& proxy_eid, const OctetStr& engine_id) {
    ...
    ind = Oidx::from_string("private", FALSE);
    r = snmpCommunityEntry->find_index(ind);
    if (!r) r = snmpCommunityEntry->add_row(ind);
    snmpCommunityEntry->set_row(r,
                OctetStr("private"),
                OctetStr("coex-private"),
                engine_id,
                OctetStr(""),
                OctetStr("access"),
                3, 1);
...
void init(Mib& mib, const NS_SNMP OctetStr& engineID) {
    ...
    initCommunities(&mib, proxy_eid, engineID);
...
int main(int argc, char* argv[]) {
    ...
    vacm->addNewGroup(SNMP_SECURITY_MODEL_V1, "public",
            "v1v2group", storageType_nonVolatile);
    vacm->addNewGroup(SNMP_SECURITY_MODEL_V2, "coex-private",
            "v1v2group", storageType_nonVolatile);
    vacm->addNewGroup(SNMP_SECURITY_MODEL_V1, "coex-private",
            "v1v2group", storageType_nonVolatile);
