...
int main (int argc, char* argv[])
{
	...
	DefaultLog::log()->set_filter(EVENT_LOG, 6);
	DefaultLog::log()->set_filter(INFO_LOG, 5);
	DefaultLog::log()->set_filter(DEBUG_LOG, 4);
#endif
	int status;
    ...
    vacm->addNewGroup(SNMP_SECURITY_MODEL_V2, "community-public",
                     "v1v2group", storageType_nonVolatile);
    vacm->addNewGroup(SNMP_SECURITY_MODEL_V1, "community-public",
                     "v1v2group", storageType_nonVolatile);
    ...
    vacm->addNewAccessEntry("newGroup",
			...
			match_exact,  // context must match exactly
	...
	OctetStr co("public");
	OctetStr secName("community-public");
	...
	no.add_v1_trap_destination(dest, "defaultV1Trap", "v1trap", "community-public");
